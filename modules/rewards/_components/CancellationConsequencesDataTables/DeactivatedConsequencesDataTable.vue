<template>
  <div class="table-container">
    <el-table
      :data="deactivatedCancellationConsequences"
      height="500"
      size="medium"
      :border="false"
    >
      ...
      <el-table-column label="Country" prop="country">
        <template slot-scope="scope">
          {{
            fetchCountry(
              deactivatedCancellationConsequences[scope.$index]['country_code'],
            )
          }}
        </template>
      </el-table-column>
          ...
          <span class="stop-word-break">
            {{
              formatVendorTypeNames(
                deactivatedCancellationConsequences[scope.$index][
                  'vendor_type_names'
                ],
              )
            }}
      ...
      <el-table-column label="Actions" width="200" prop="actions">
        <template slot-scope="scope">
          <span class="stop-word-break">
            {{
              getActionTypes(
                deactivatedCancellationConsequences[scope.$index]['actions'],
              )
            }}
          </span>
        </template>
      ...
      <el-table-column label="Date added" prop="date_created">
        <template slot-scope="scope">
          {{
            getFormattedDate(
              deactivatedCancellationConsequences[scope.$index]['date_created'],
              'DD/MM/YYYY ',
            )
          }}
        </template>
      ...
      <el-table-column label="Status" prop="status">
        <template slot-scope="scope">
          {{
            activeStatus(
              deactivatedCancellationConsequences[scope.$index]['status'],
            )
          }}
        </template>
      </el-table-column>
      <el-table-column label="Actions" prop="status" class="data" width="150">
        <template slot-scope="scope">
          <el-button
            size="mini"
            :class="
              deactivatedCancellationConsequences[scope.$index]['status'] === 1
                ? 'action-button--danger'
                : 'action-button--active'
            "
            @click="
              setStatusState(deactivatedCancellationConsequences[scope.$index])
            "
          >
            {{
              loading(deactivatedCancellationConsequences[scope.$index]['id'])
                ? 'Processing...'
                : setStatusText(
                    deactivatedCancellationConsequences[scope.$index],
                  )
  ...
  computed: {
    ...mapGetters({
      deactivatedCancellationConsequences:
        'getDeactivatedCancellationConsequences',
    }),
  },
};
</script>
